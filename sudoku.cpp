...
class Possible {
    ...
    public:
    // _b has 9 elements with 'true' value, equal to "123...9"
    Possible() : _b(9, true) {}
    bool is_on(int i) const {
        return _b[i-1];
    }

    // std::count() returns numbers of occurrences of an element in a given
    // range. Returns the number of elements in the range [first, last) that
    ...
    int val() const {
        // Finds the element in the given range of numbers. Returns an
        // iterator to the first element in the range [first,last) that
...
void Sudoku::init() {
    ...
    for (int k = 0; k < _neighbors.size(); k++) {
        for (int x = 0; x < _groups_of[k].size(); x++) {
            for (auto i : _group[_groups_of[k][x]])
                if (i != k)
                    _neighbors[k].push_back(i);
        }
    }
}
...
// Heuristic search, choose the cell wich the minimum number of possible
// values.
...
// A unique_ptr "owns" the object to which it points, only one unique_ptr at a
// time can point to a given object. The object to which a unique_ptr points
// is destroyed when the unique_ptr is destroyed.
